...
using namespace Slate::Language::Expansions;
...
void test_dot_operation()
{
    static_assert(std::is_same_v<Dot_A, 
        Wrap<Wrap<A, Wrap<>, Wrap<C>>, Wrap<A, Wrap<>, Wrap<E>>>>, 
...
void test_state_operation()
{
    static_assert(requires(){
        typename State_A::Next<C>;
    }, "State A does not have the C transition");

    static_assert(requires(){
        typename State_A::Next<B>;
    }, "State A does not have the B transition");
}
